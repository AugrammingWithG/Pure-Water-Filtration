...
Everything is rounded and nothing is sharp. Buttons, chips, tags, badges and the quick-quote are full pills (999px). Cards are 22 to 30px: 24px is the default (`--radius`), 30px for hero-scale frames (viewer card, Decoder panel), 22px for review cards, 18px for benefit tiles, 20px for journey and guide cards. The one deliberately tighter place is the Water Lab section, which is an instrument rather than a card: 14px on its live frame and its three control cards, 8px on the glass tags and the system buttons, 10px on its full-Lab button, 6px on the stage chips. Small controls step down: choices 15px, FAQ items 16px, toast 14px, Lab panels 14px, Lab chips and placement callouts 8px, mobile action buttons 10px. Circles are used for icons, step numbers, trust icons, the modal close and the rail icon backgrounds (11 to 12px on a 38px square, not fully round).
...
- **Lab card** (Water Lab section): a control, not a caption — the system picker, the play button with its stage timeline, the live "removes" chips. White 6.5→3% gradient, 1px `rgba(159,222,246,.16)`, 14px, 20px 22px padding, a hairline of cyan light along the top edge; hover lifts 3px and brightens the border to 40% cyan. Beside it the **Lab frame**: the live diorama at 14px with a 1px hairline, glass tags (`rgba(6,27,46,.68)`, blur 10px, 8px) in its corners, a light that sweeps once round its rim as the section arrives, and the poster blurring away under the canvas once its shaders are built
...
### Motion
The page snaps section to section (`scroll-snap-type: y mandatory` on the root, each `.section` a `start` stop sat under the sticky nav); one flick lands on the next section. Reveals rise 28px over `.8s` on `cubic-bezier(.2,.7,.2,1)` with 80ms stagger steps (`.delay1` to `.delay5`), or slide in from a side (`.from-left`, `.from-right`) or settle from 94% (`.zoom`); a `.reveal-stagger` block lands its own lines one after another, 90ms apart. Each section, once `.in-view`, choreographs its pieces (step numbers and stars pop on `cubic-bezier(.34,1.56,.64,1)`, the decoder track grows, the map pins drop in west to east, table rows and FAQ items cascade). All entrances are keyframes with a `backwards` fill on `translate`/`scale`, never `transform`, so hover transforms are untouched. Hovers transition `.25 to .45s`; the quick-quote slides in over `.45s cubic-bezier(.2,.8,.2,1)`; ambient glows breathe on 5 to 8s ease-in-out loops; map pins ping on 2.2s. `body.reduced-motion` and `prefers-reduced-motion` collapse all of it to 1ms and turn the snapping off. The 3D canvas fades in over `.9s` (page) or `.6s` (Lab) only once its shaders are built.
